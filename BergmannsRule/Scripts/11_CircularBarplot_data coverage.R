...
# birds ####
ET<-read.csv('Data/BirdFuncDat.csv', stringsAsFactors = F)
ET<-ET[!ET$PelagicSpecialist %in% 1,] # remove Pelagic species
ET<-ET[!ET$BLFamilyLatin %in% 'Pelecanidae',]

birddata <- read.csv('Data/birddata_ph.csv', stringsAsFactors = F)
birddata <- birddata[birddata$env.var == 'tavg', ]

ET2<-ET[!duplicated(ET$BLFamilyLatin),] # 184 families
birddata2<-birddata[!duplicated(birddata$family),] # 140 families (140/184*100 = 76%)

TabET<-table(ET$IOCOrder) #n species
TabETf<-table(ET2$IOCOrder) #n families
TabSP<-table(birddata$order) #n species
TabSPf<-table(birddata2$order) #n families

ET<-data.frame(Order=names(TabET), Nfm=as.numeric(TabETf), Nsps=as.numeric(TabET))
bird_dt<-data.frame(Order=names(TabSP), NfmInSamp=as.numeric(TabSPf), NspInSamp= as.numeric(TabSP))

data<-left_join(ET, bird_dt, by = 'Order')
data$NspInSamp[is.na(data$NspInSamp)]<-0
data$NfmInSamp[is.na(data$NfmInSamp)]<-0
...
#SAVE ALL
pdf('Figures/FamilyCoverageBirds.pdf', width=18, height=12)#width=13, height=8)
ggarrange(p2, p1, labels=c('(a)', '(b)'), font.label=list(size=20, face='plain'),
          nrow=2, ncol=2)
dev.off()
