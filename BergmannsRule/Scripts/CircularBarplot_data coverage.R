...
DT<-read.csv('~/Documents/Progetti/Density/MAMMALS/TRAITS_Mammals.csv', sep=';')
D<-read.csv('~/Documents/Progetti/DENSITY/MAMMALS.csv')
D<-merge(D, DT[,c('sp_id','Order')], by='sp_id', all.x=TRUE)
...
ET2<-ET[!duplicated(ET$MSWFamilyLatin),]
DT2<-DT[!duplicated(DT$Family),]

TabET<-table(ET$Order) #n species
TabETf<-table(ET2$Order) #n families
TabSP<-table(DT$Order) #n species
TabSPf<-table(DT2$Order) #n families

ET<-data.frame(Order=names(TabET), Nfm=as.numeric(TabETf), Nsps=as.numeric(TabET))
DT<-data.frame(Order=names(TabSP), NfmInSamp=as.numeric(TabSPf), NspInSamp=as.numeric(TabSP))

data<-merge(ET, DT, by='Order', all.x=TRUE)
...
#SAVE ALL
pdf('~/Documents/Progetti/Density/ImprovingPredictions/MS/GEB/MR/Minors/Figs/Fig. 1 Coverage.pdf', width=18, height=12)#width=13, height=8)
ggarrange(p2, p1, p3, p4, labels=c('(a)', '(b)', '(c)', '(d)'), font.label=list(size=20, face='plain'),
  nrow=2, ncol=2)
dev.off()
